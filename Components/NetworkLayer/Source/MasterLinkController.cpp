 ...
 *
 ****************************************************************************/
MasterLinkController::MasterLinkController(const QString &ip, const QString &port) :
        m_myNetClient(NULL),
        m_IP(ip),
        m_Port(port)
{
...
bool MasterLinkController::Initialize()
{
    try {
        ...
        m_myNetClient = new NetworkBase::NetworkClientDevice(MY_CLIENT_TYPE, m_IP, m_Port, "", NULL);
    }
